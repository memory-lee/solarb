import { TOKEN_PAIRS, createP2PNode, subscribePrices, } from "@solarb/shared";
 ...
 */
function logAnalysis(opportunities) {
    ...
    for (const [pair, opps] of byPair) {
        ...
        if (th) {
            console.log(`    threshold: ${th.threshold.toFixed(4)}% (mean=${th.mean.toFixed(4)}%, std=${th.std.toFixed(4)}%, samples=${th.samples})`);
        }
    }
    ...
    if (significant.length > 0) {
        ...
        for (const opp of significant) {
            saveOpportunity(opp).catch((err) => console.error("[DynamoDB] Save failed:", err));
            sendAlert(opp).catch((err) => console.error("[SNS] Alert failed:", err));
        }
    }
}
/**
...
async function main() {
    ...
    console.log(`[SolArb Detector] Monitoring ${TOKEN_PAIRS.length} pairs: ${TOKEN_PAIRS.map((p) => p.name).join(", ")}`);
    console.log(`[SolArb Detector] Adaptive threshold: mean + ${2}σ, min=${0.005}%`);
    try {
