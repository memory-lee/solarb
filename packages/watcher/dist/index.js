import { TOKEN_PAIRS, POLL_INTERVAL_MS, createP2PNode, publishPrices, subscribePrices, } from "@solarb/shared";
...
async function pollPrices() {
    const allPrices = [];
    const results = await Promise.allSettled(TOKEN_PAIRS.map((pair) => fetchJupiterQuotes(pair)));
...
function buildPriceMessage(prices) {
    return {
        type: "price_update",
        nodeId: NODE_ID,
        prices,
